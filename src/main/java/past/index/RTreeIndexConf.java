...
public class RTreeIndexConf implements Serializable {
	...
	/**
	 * Computes the vector associated to a pointer, based on the current dataset.
	 * Warning : NOT to be used inside a spark worker's code.
	 * 
	 * @param pointer pointer of the vector
	 * @return the vector
	 */
	public int[] getVectorByPointer(String pointer) {
		List<int[]> valuesOfKey = this.dataset.lookup(pointer);
		if (valuesOfKey.size() != 1) {
			throw new AssertionError("There can be only one value associated to a key in the dataset.");
		}
		
		return valuesOfKey.get(0);
	}
	
	/**
	 * WARNING : this method does not work, the RDD has to be correctly generated by the storage
     *
	 * @param timeseries
	 * @param columnNames
	 */
